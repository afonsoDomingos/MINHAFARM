...
export default function PharmacyMedicinesPage() {
  ...
  const fetchLowStock = async () => {
    try {
      const response = await fetch('/api/pharmacies/my-pharmacy/low-stock');
      if (response.ok) {
          ...
          <div className="mb-4 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
            ...
            <ul className="mt-2 list-disc list-inside">
              {lowStockItems.slice(0, 5).map((item) => (
                <li key={item.id}>
                  {item.name} - Restam: {item.quantity} unidades
                </li>
              ))}
              {lowStockItems.length > 5 && (
                <li className="text-sm">...e mais {lowStockItems.length - 5} produtos</li>
              )}
            </ul>
          </div>
        )}
